...
# arrangement exists to avoid. This repo runs Dependabot (GitHub-primary),
# which can't act on a Renovate-style regex-manager comment, so this pin is
# bumped by hand.
VERSION=18.7.0
...
# CHANGELOG.md is written by the release job, not by hand. OpenSpec's
# structured requirement/scenario format reads nothing like this repo's own
# docs voice, so it's out of scope for this pipeline (see
# openspec/changes/add-markdown-prose-linting/proposal.md). The PR template
# is a GitHub compose-box fragment, not a standalone document.
files=$(git ls-files '*.md' | grep -v '^CHANGELOG.md$' | grep -v '^openspec/' | grep -v '^\.github/PULL_REQUEST_TEMPLATE\.md$')

# A checker fed an empty file list is a usage error easily misread as "found
# problems" (or silently as a pass) — fail loud instead of guessing which.
if [ -z "$files" ]; then
